...
public:
	...
	bool LoadFromResource(unsigned int id)
	{
		...
		std::istringstream iss(res_data);
		std::istream *is = &iss;
...
public:
	...
	void DrawWireFrameModel(const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y, float r = 0.0f, float s = 1.0f, short col = FG_WHITE, wchar_t c = PIXEL_SOLID);
	int ScreenWidth();
	int ScreenHeight();
